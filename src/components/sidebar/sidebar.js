            ...
            <div className='sidebar--course-name'>
                <div className='sidebar--course-table_logout'>
                  { content.courses.map((course) => (
                    <Link to={ `/departments/${content.activeDepartment.abbr}/courses/${course.code}/` } key={ course.id }>
                        <CourseHandle login={false} code={course.code} title={course.title} course={course.id}/>
                    </Link>
                  )) }
                </div>
            </div>
